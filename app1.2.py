...
<style>
    ...
    .main-header {
        ...
        color: white; box-shadow: 0 8px 32px rgba(192,57,43,0.3);
    }
    ...
    .metric-card {
        ...
        box-shadow: 0 2px 16px rgba(0,0,0,0.08); border-left: 5px solid #c0392b; margin-bottom: 0.8rem;
    }
    ...
    .stButton > button {
        ...
        padding: 0.6rem 2rem !important; font-size: 1.05rem !important; font-weight: 600 !important;
...
NEEDS_SCALING = {"Logistic Regression": True, "SVM": True, "Decision Tree": False, "Random Forest": False}
MODEL_COLORS  = {"Logistic Regression": "#4C72B0", "SVM": "#DD8452", "Decision Tree": "#55A868", "Random Forest": "#C44E52"}
...
input_dict = {
    ...
    "physical_activity": pa_enc, "diet_quality": diet_enc, "stress_level": stress,
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab1, tab2, tab3, tab4 = st.tabs(["🔮 Prediction", "📊 Model Comparison", "📈 Feature Analysis", "ℹ️ About"])
...
with tab1:
    if predict_btn:
        ...
        if selected_prob >= 0.65:
            ...
            risk_level, risk_class, risk_icon = "MODERATE RISK", "risk-moderate", "🟡"
        else:
        ...
        with col_res2:
            fig_gauge = go.Figure(go.Indicator(
                mode="gauge+number+delta", value=selected_prob * 100,
                ...
                gauge={
                    ...
                    "steps": [{"range": [0,35], "color": "#d1fae5"}, {"range": [35,65], "color": "#fef3c7"}, {"range": [65,100], "color": "#fee2e2"}],
        ...
        for i, (mname, res) in enumerate(all_preds.items()):
            with cols[i]:
                ...
                st.markdown(f'<div class="metric-card" style="border-left-color:{color}"><h3>{mname}</h3><h2 style="color:{color}">{pct:.1f}%</h2><p style="margin:0;font-size:0.85rem;font-weight:600">{label}</p></div>', unsafe_allow_html=True)

        prob_names = list(all_preds.keys())
        prob_vals  = [all_preds[n]["prob"] * 100 for n in prob_names]
        fig_bar = go.Figure(go.Bar(x=prob_names, y=prob_vals, marker_color=[MODEL_COLORS[n] for n in prob_names], text=[f"{v:.1f}%" for v in prob_vals], textposition="outside"))
        fig_bar.add_hline(y=50, line_dash="dash", line_color="black", annotation_text="Decision Threshold (50%)")
        fig_bar.update_layout(title="Risk Probability by Model", yaxis_title="Risk Probability (%)", yaxis_range=[0,110], height=350, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig_bar, use_container_width=True)

        st.markdown('<div class="section-title">💊 Clinical Recommendations</div>', unsafe_allow_html=True)
        recommendations = []
        if systolic_bp >= 140 or diastolic_bp >= 90: recommendations.append("🔴 **Hypertension detected** — consult a physician for BP management")
        if total_cholesterol > 240: recommendations.append("🔴 **High total cholesterol** — dietary changes and possible medication needed")
        if ldl > 160: recommendations.append("🔴 **Elevated LDL** — consider statin therapy evaluation")
        if fasting_glucose > 126 or hba1c > 6.5: recommendations.append("🔴 **Diabetes indicators** — endocrinology referral recommended")
        if bmi >= 30: recommendations.append("🟡 **Obesity (BMI ≥ 30)** — structured weight management program")
        if smoking == "Yes": recommendations.append("🟡 **Smoking cessation** — significantly reduces cardiovascular risk")
        if physical == "Low": recommendations.append("🟡 **Low physical activity** — 150 min/week moderate exercise recommended")
        if stress > 7: recommendations.append("🟡 **High stress level** — stress management/counselling advised")
        if hdl < 40: recommendations.append("🟡 **Low HDL** — increase physical activity, reduce trans fats")
        if family_hist: recommendations.append("ℹ️ **Family history present** — more frequent cardiac screening advised")
        if not recommendations: recommendations.append("✅ **No critical risk factors identified** — continue healthy lifestyle habits")
        for rec in recommendations:
            st.markdown(f"- {rec}")
        st.markdown('<div class="disclaimer">⚠️ <b>Medical Disclaimer:</b> This tool is for educational/research purposes only. It does NOT replace professional medical advice, diagnosis, or treatment. Always consult a qualified healthcare provider.</div>', unsafe_allow_html=True)
    else:
...
with tab2:
    ...
    if os.path.exists(csv_path):
        ...
        st.dataframe(display_df.style.highlight_max(axis=0, color="#d1fae5").format("{:.4f}"), use_container_width=True)
        ...
        for mname in model_names_r:
            vals = comp_df.loc[mname, available].tolist() + [comp_df.loc[mname, available[0]]]
            fig_radar.add_trace(go.Scatterpolar(r=vals, theta=available+[available[0]], fill="toself", name=mname, line_color=MODEL_COLORS.get(mname,"gray"), opacity=0.6))
        fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0,1])), showlegend=True, title="Model Performance Radar Chart", height=450)
        st.plotly_chart(fig_radar, use_container_width=True)
        fig_gb = go.Figure()
        for metric in available:
            fig_gb.add_trace(go.Bar(name=metric, x=model_names_r, y=comp_df[metric].tolist(), text=[f"{v:.3f}" for v in comp_df[metric]], textposition="outside"))
        fig_gb.update_layout(barmode="group", title="All Metrics — All Models", yaxis_title="Score", yaxis_range=[0,1.12], height=400, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig_gb, use_container_width=True)
    else:
        st.warning("Run `python train_models.py` to generate comparison data.")
    for img_file, title in [("roc_curves.png","ROC Curves"),("pr_curves.png","Precision-Recall Curves"),("cv_boxplot.png","Cross-Validation Scores"),("confusion_matrices.png","Confusion Matrices")]:
...
with tab3:
    ...
    if predict_btn:
        ...
        radar_features = {"Age": min(age/90,1.0), "BMI": min(bmi/45,1.0), "Systolic BP": min(systolic_bp/200,1.0), "LDL": min(ldl/250,1.0), "Glucose": min(fasting_glucose/300,1.0), "HbA1c": min(hba1c/14,1.0), "CRP": min(crp/20,1.0), "Stress": stress/10, "Triglycerides": min(trig/600,1.0)}
        labels  = list(radar_features.keys())
        values  = list(radar_features.values()) + [list(radar_features.values())[0]]
        fig_pt  = go.Figure()
        fig_pt.add_trace(go.Scatterpolar(r=values, theta=labels+[labels[0]], fill="toself", name="Patient", line_color="#c0392b", fillcolor="rgba(192,57,43,0.2)"))
        fig_pt.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0,1])), title="Patient Risk Factor Radar (Normalised)", height=420)
        st.plotly_chart(fig_pt, use_container_width=True)
        binary_data = {"Risk Factor": ["Diabetes","Hypertension","Family History","Prev. Heart Disease","Stroke History","Kidney Disease","Abnormal ECG","LV Hypertrophy","Chest Pain","Shortness of Breath","Fatigue","Dizziness","Smoking"], "Present": [diabetes,hypertension,family_hist,prev_heart,stroke_hist,kidney_dis,ecg_abn,lvh,chest_pain,sob,fatigue,dizziness,smoking=="Yes"]}
        ...
        st.metric("Total Risk Factors Present", f"{b_df['Present'].sum()} / {len(b_df)}")
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .inner {{ padding: 28px 32px 22px 32px; }}

  .brand-row {{ display: flex; align-items: center; gap: 10px; margin-bottom: 4px; }}
  .brand-icon {{ font-size: 20px; animation: beat 1.4s ease-in-out infinite; line-height: 1; }}
  .brand-name {{ font-size: 20px; font-weight: 700; color: #fff; }}
  .brand-tag {{ font-size: 10px; color: #a8d8ea; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 24px; }}
  ...
  .col-title {{ font-size: 9px; font-weight: 700; letter-spacing: 2.5px; text-transform: uppercase; color: #c0392b; margin-bottom: 10px; }}
  .col p {{ font-size: 12px; color: #cbd5e1; line-height: 1.75; }}
  .warn {{ font-size: 11px; color: #94a3b8; line-height: 1.6; margin-top: 8px; }}
  .col ul {{ list-style: none; }}
  .col li {{ font-size: 12px; color: #cbd5e1; line-height: 2; }}
  .col li::before {{ content: "› "; color: #8e44ad; font-weight: 700; }}

  .pills {{ display: flex; flex-wrap: wrap; gap: 5px; margin-top: 12px; }}
  .pill {{ background: rgba(192,57,43,0.18); border: 1px solid rgba(192,57,43,0.4); color: #fca5a5; font-size: 10px; font-weight: 600; padding: 3px 10px; border-radius: 20px; }}

  hr {{ border: none; border-top: 1px solid rgba(255,255,255,0.09); margin: 0 0 16px; }}

  .bottom {{ display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; }}
  .copy {{ font-size: 11px; color: #94a3b8; }}
  .copy strong {{ color: #c0392b; }}
  .beat-icon {{ display: inline-block; animation: beat 1.4s ease-in-out infinite; }}
  ...
  @media (max-width: 420px) {{
    ...
    .col-about {{ grid-column: auto; }}
    .inner {{ padding: 20px 18px 16px 18px; }}
  ...
  <div class="inner">
    ...
    <div class="grid">
      <div class="col col-about">
        ...
        <p>Heartify is an AI-powered cardiovascular risk assessment tool built as an academic research project. It analyses 31 clinical, lifestyle, and demographic features to estimate heart attack probability using four ML classifiers trained on 20,000 patient records.</p>
        <p class="warn">⚠️ For educational &amp; research use only. Not a substitute for professional medical advice.</p>
      </div>
      <div class="col">
      ...
      <div class="col">
        ...
        <ul>
          ...
          <li>20,000 patient records</li>
          <li>31 input variables</li>
          <li>Best: Logistic Regression</li>
    ...
    <div class="bottom">
      <div class="copy">
        <span class="beat-icon">❤️</span>&nbsp;
        © {current_year} <strong>Heartify</strong> — All rights reserved.
      </div>
      <div class="rights">
...
<script>
  function resizeIframe() {{
    const h = document.getElementById('footer').getBoundingClientRect().height;
    // Tell the parent Streamlit page the exact height needed
  ...
  // Re-run if window is resized (e.g. sidebar open/close)
